...
function caption() {
  const logoSize = 84;
  const logoScale = logoSize / 282;
  return `<g font-family="JetBrains Mono" font-size="28">` +
    `<g transform="translate(28 ${OG_HEIGHT - 28 - logoSize}) scale(${logoScale.toFixed(4)}) translate(-243 -243)">` +
    `<path d="${LOGO_PATH}" fill="${THEME.captionStrong}"/></g>` +
    `<text x="${OG_WIDTH - 28}" y="${OG_HEIGHT - 28}" text-anchor="end" fill="${THEME.captionStrong}">ratspeak.org</text>` +
    `</g>`;
}
