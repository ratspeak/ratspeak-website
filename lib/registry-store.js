...
// Version files are written once under a unique key and never overwritten,
// so a CDN hit is always correct; a cache-busted miss is a billed operation.
export async function fetchImmutableJson(url) {
...
export async function deleteBlobs(blobToken, urls) {
...
export async function readVersioned(blobToken, secret, prefix, legacyPath) {
  ...
  if (newest) return openSealedValue(secret, await fetchImmutableJson(newest.url));
  return legacyPath ? readSealed(blobToken, secret, legacyPath) : null;
}

// Groups a listing under `prefix` by record id (`<id>/<version>.json` or the
// legacy `<id>.json`) and splits it by the newest blob's age: `live` groups
// still need a read, `dead` ones are older than any actionable record and
// safe to delete. Age comes from the listing so no blob is fetched to decide.
export function triageVersionedGroups(blobs, prefix, { liveMs, deadMs, now = Date.now() }) {
  ...
  for (const blob of blobs) {
    ...
    if (!id) continue;
    const entry = groups.get(id) || { id, legacy: null, versions: [], newestAt: 0 };
    if (version) entry.versions.push(blob);
    else entry.legacy = blob;
    entry.newestAt = Math.max(entry.newestAt, blobTimestamp(blob));
    groups.set(id, entry);
  }
  const live = [];
  const dead = [];
  for (const entry of groups.values()) {
    ...
    entry.newest = entry.versions[entry.versions.length - 1] || entry.legacy;
    const age = entry.newestAt ? now - entry.newestAt : 0; // unknown age reads as live
    if (age > deadMs) dead.push(entry);
    else if (age <= liveMs) live.push(entry);
  }
  return { live, dead };
}
...
function blobTimestamp(blob) {
  ...
  const key = /\/(\d{14})-[0-9a-f]+\.json$/.exec(blob.pathname || '');
  return key ? Number(key[1]) : 0;
}
