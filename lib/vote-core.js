...
export const STARTER_POLL_ID = 'community-improvements';
export const STARTER_POLL_TITLE = 'Which community improvements next?';
export const STARTER_POLL_CLOSES = '2026-06-07T23:59:00Z';
export const STARTER_POLL_WINNER_CHOICE_ID = 'distribute-more-nodes';

const LEGACY_STARTER_POLL_ID = 'node-deployments';
const STARTER_CHOICE_IDS_BY_LEGACY = {
  'rural-us-mesh-seed-kits': 'distribute-more-nodes',
  'hackerspaces-and-maker-labs': 'create-youtube-videos-and-blogs',
  'global-firmware-testers': 'improve-discord-and-telegram',
  'university-radio-clubs': 'marketing-materials'
};
const STARTER_CHOICE_IDS_BY_NAME = {
  'distribute more nodes': 'distribute-more-nodes',
  'create youtube videos and blogs': 'create-youtube-videos-and-blogs',
  'improve discord & telegram': 'improve-discord-and-telegram',
...
export const DEFAULT_POLLS = [
  {
    id: STARTER_POLL_ID,
    title: STARTER_POLL_TITLE,
    deckTitle: 'Community Allocation',
    type: 'Proposal #1',
    snapshotBlock: SNAPSHOT_BLOCK,
    closes: STARTER_POLL_CLOSES,
    description: 'Help us decide where to improve the community next!',
    winnerChoiceId: STARTER_POLL_WINNER_CHOICE_ID,
    choices: [
      { id: 'distribute-more-nodes', name: 'Distribute more nodes', detail: 'Give away more nodes to the community worldwide.' },
      { id: 'create-youtube-videos-and-blogs', name: 'Create YouTube videos and blogs', detail: 'Create educational and informational videos on Ratspeak & Reticulum.' },
      { id: 'improve-discord-and-telegram', name: 'Improve Discord & Telegram', detail: 'Get Discord and Telegram better setup to support crypto community.' },
      { id: 'marketing-materials', name: 'Marketing materials', detail: 'Create one-pagers, flyers, etc., that can easily be distributed and shared.' }
    ]
  }
];
...
export function normalizePoll(poll, index = 0) {
  ...
  const title = cleanText(poll.title, 'Untitled proposal', 220);
  const id = canonicalStarterPollId(slugify(poll.id, `proposal-${index + 1}`), title);
  let choices = Array.isArray(poll.choices)
    ? poll.choices.map((choice, choiceIndex) => normalizeChoice(choice, choiceIndex)).filter(Boolean)
    : [];
  if (choices.length < 2) return null;
  if (id === STARTER_POLL_ID) {
    choices = choices.map(canonicalStarterChoice);
  }
  const isStarterPoll = id === STARTER_POLL_ID;
  const winnerId = isStarterPoll ? STARTER_POLL_WINNER_CHOICE_ID : slugify(poll.winnerChoiceId, '');

  const snapshotBlock = Number(poll.snapshotBlock);
  return {
    id,
    title: isStarterPoll ? STARTER_POLL_TITLE : title,
    ...
    closes: isStarterPoll ? STARTER_POLL_CLOSES : normalizeCloseTime(poll.closes),
    description: cleanText(poll.description, '', 280),
    winnerChoiceId: choices.some(choice => choice.id === winnerId) ? winnerId : '',
    choices,
...
function canonicalStarterPollId(id, title) {
  if (id !== LEGACY_STARTER_POLL_ID) return id;
  const normalizedTitle = String(title || '').trim().toLowerCase();
  if (
    normalizedTitle === 'which community improvements next?' ||
    normalizedTitle === 'what community improvements next?'
  ) {
    return STARTER_POLL_ID;
  }
  return id;
}

function canonicalStarterChoice(choice) {
  const nameKey = String(choice.name || '').trim().toLowerCase();
  return {
    ...choice,
    id: STARTER_CHOICE_IDS_BY_LEGACY[choice.id] || STARTER_CHOICE_IDS_BY_NAME[nameKey] || choice.id
  };
}
...
// The declared winner of a closed poll, or null. Winners are stored per poll
// (`winnerChoiceId`, set by an admin after close), never inferred from live
// tallies, so an open poll or an unset winner yields null.
export function pollWinnerChoice(poll, nowMs = Date.now()) {
  if (!poll?.winnerChoiceId || !isPollClosed(poll, nowMs)) return null;
  return findChoice(poll, poll.winnerChoiceId);
}
...
export function aggregateResults(poll, votes, options = {}) {
  ...
  for (const vote of votes) {
    const canonicalVote = canonicalVoteForPoll(vote, poll);
    if (!canonicalVote) continue;
    const voterKey = String(canonicalVote.voter || '').toLowerCase();
    if (!voterKey) continue;
    const decimals = Number.isInteger(canonicalVote.tokenDecimals) ? canonicalVote.tokenDecimals : tokenDecimals;
    if (!isEligibleVote(canonicalVote, decimals)) continue;
    const existing = latestByVoter.get(voterKey);
    if (!existing || compareVoteFreshness(canonicalVote, existing) >= 0) {
      latestByVoter.set(voterKey, canonicalVote);
    }
  }
...
export function canonicalVoteForPoll(vote, poll) {
  if (!vote || !poll) return null;
  let pollId = String(vote.pollId || '');
  let choiceId = String(vote.choiceId || '');
  if (poll.id === STARTER_POLL_ID && pollId === LEGACY_STARTER_POLL_ID) {
    pollId = STARTER_POLL_ID;
    choiceId = STARTER_CHOICE_IDS_BY_LEGACY[choiceId] || choiceId;
  }
  if (pollId !== poll.id) return null;
  if (Number(vote.snapshotBlock) !== Number(poll.snapshotBlock)) return null;
  return { ...vote, pollId, choiceId };
}

export function voteStoragePollIds(poll) {
  if (!poll?.id) return [];
  if (poll.id === STARTER_POLL_ID) return [STARTER_POLL_ID, LEGACY_STARTER_POLL_ID];
  return [poll.id];
}

function compareVoteFreshness(a, b) {
