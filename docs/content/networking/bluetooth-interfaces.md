...
The BLE RNode bridge is the wireless equivalent of plugging a radio into a USB port. The phone or laptop is the BLE central; the RNode (Ratdeck, Ratcom, or any RNode-firmware board with the BLE option compiled in) is the peripheral. KISS frames — the same byte stream that runs over USB serial — are tunneled through a GATT characteristic instead of a UART. The result is point-to-point: one phone, one radio.
...
1. Put the RNode into BLE pairing mode. On boards with a screen this is usually a long-press of the left button until the OLED shows a 6-digit passkey. Boards without screens (Ratdeck, Ratcom) advertise immediately on first boot or after a reset.
...
The peer mesh is a phone-to-phone (or phone-to-laptop) Reticulum link carried directly over Bluetooth Low Energy GATT. Every Ratspeak node running with BLE enabled is **both** an advertiser and a scanner — the roles are symmetric. When two nodes see each other, the one with the lexicographically lower identity hash takes the central role and opens the connection. This deterministic tiebreak prevents both sides from connecting at once.
...
- **Ratspeak peer mesh** — `a1b2c3d4-e5f6-4a5b-8c9d-0e1f2a3b4c5d`
- **Columba compatibility** — `37145b00-442d-4a94-917f-8f42c5da28e3` (advertised in parallel for cross-app interop with Columba clients)

Use the peer mesh as a "right-here-with-you" sideband: a few people in the same room, a vehicle convoy, a small group on a hike, or a backstage crew. It is not a long-distance transport — it is the layer that makes nearby devices reachable when LoRa is overkill and Wi-Fi is unavailable or untrusted.

### Enabling it

Open Network → **BLE Peer Mesh** and toggle it on. The state persists, so the mesh comes back up on next launch. No configuration file edits, no daemon restart. Discovery is continuous in the background; peers appear in the network graph as they come into range.

The mesh registers as a normal Reticulum interface, which means announces, links, resources, and LXMF traffic all flow through it the same way they would over LoRa or TCP. A peer that is reachable both over BLE and over LoRa will be picked over BLE when it is closer in hop cost — the transport layer chooses, not the user.
...
The BLE peer mesh works on iOS as well, but Core Bluetooth applies stricter background-execution limits than Android — extended advertising while the app is fully suspended is not guaranteed. Keep Ratspeak in the foreground or in the recents tray during active sessions for reliable peer discovery. Granting the *Bluetooth* permission at first launch is required; without it both the bridge and the peer mesh will silently find nothing.
...
- **Concurrency** — most phone radios will hold a handful of simultaneous BLE connections cleanly. Beyond that, the OS starts dropping links unpredictably. Treat the peer mesh as small-group transport — five to eight nearby nodes is realistic, twenty is not.
...
- **Battery** — continuous scanning + advertising is the most power-hungry mode BLE has. Expect a noticeable drain on phones left running the peer mesh all day; the bridge alone, with no scanning, is much cheaper.
...
If something is misbehaving, the Network view in Ratspeak shows live counters per interface: bytes in, bytes out, last frame, current peer count. A BLE interface that is up but reading zero bytes is almost always a permission issue or a stale OS-level bond, not a bug in the radio or the mesh.
