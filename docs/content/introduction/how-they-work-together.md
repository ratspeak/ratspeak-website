...
| **LRGP** | Games — multiplayer sessions encoded in messages | A form inside the envelope |
| **Ratspeak** | Interface — dashboard, controls, visualization | Your desk |

Each layer depends on the one below it. Reticulum handles the network plumbing. LXMF handles message formatting and delivery. LRGP lets games embed multiplayer sessions inside messages. Ratspeak ties it all together in a browser-based interface you can actually use.
...
- **Reticulum** (network layer) — handles identity, addressing, encryption, routing, and transport across any combination of physical media
- **LXMF** (messaging layer) — adds reliable delivery, store-and-forward propagation, and message semantics on top of Reticulum links
- **Ratspeak** (application layer) — provides the user interface (dashboard, messaging, contacts, network visualization) and application-specific features like LRGP games
...
| Best for | Getting started, broad platform support | Performance, lower resource usage |
