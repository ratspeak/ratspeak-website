...
Both standalone packages are at v0.3.0 and MIT-licensed; the version that ships in Ratspeak inherits Ratspeak's AGPL-3.0 license. The `[chess]` extra on the Python package pulls in `python-chess` (the Rust crate uses `cozy-chess` directly).
